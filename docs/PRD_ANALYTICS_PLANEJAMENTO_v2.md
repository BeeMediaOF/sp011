...
## Fase 0 — Reconhecimento e auditoria

A Fase 0 tem duas etapas obrigatórias, **nesta ordem**. A separação existe
para evitar que hipóteses se formem antes do mapeamento estar completo — sem
o inventário na mão, é fácil "confirmar" a hipótese principal do Problema 1
ou 2 a partir de uma leitura parcial do código.

### Fase 0.1 — Bootstrap (mapear antes de opinar)

Antes de qualquer análise sobre os problemas críticos ou sobre a checklist,
produza `docs/prd/00-inventario.md` com:

- árvore dos três diretórios relevantes (`artifacts/api-server`,
  `artifacts/brasilia-agora`, `lib/db`), limitada aos arquivos relacionados
  a analytics/tracking/ingest/dashboard
- lista de rotas/endpoints de ingest e de dashboard identificados
  (arquivo:linha), sem julgar ainda se estão certos ou errados
- lista de eventos de tracking encontrados no client (arquivo:linha)
- localização atual do filtro de bots, da marcação `is_internal`, da
  deduplicação, da classificação de canal e da agregação de heartbeat
  (arquivo:linha), novamente sem opinar sobre correção
- schema Drizzle atual + estado do `ensureSchema.ts` — quais tabelas e
  colunas existem hoje
- lacunas de visibilidade: o que não foi possível localizar e por quê
  (arquivo grande demais, código gerado, dependência externa não legível)

**Não tire conclusões nesta etapa.** Se, ao mapear, você encontrar algo que
parece confirmar uma das hipóteses dos Problemas 1 ou 2, apenas registre a
localização — a análise vai na Fase 0.2.

### Fase 0.2 — Auditoria (análise, agora com o mapa pronto)
...
template. **Cada PRD é escrito para ser consumido pelo próprio Claude Code
em uma sessão futura de implementação, não por um leitor humano** — por
isso ele precisa ser autocontido (dá para executar lendo só o arquivo, sem
reconstruir contexto desta conversa) e cada critério tem que ser verificável
por comando ou observação objetiva, nunca por julgamento subjetivo:
...
## Comandos de verificação (rodar exatamente estes, com resultado esperado)
   Ex.:
   - `node --test artifacts/api-server/tests/ads.test.ts` → todos passam
   - `curl -s https://sp011/api/analytics/ads | jq '.impressions'` →
     valor coerente com pageviews não-internos do período
   - Consulta SQL de sanidade via MCP Supabase → 0 linhas violando
     a regra do módulo 11 correspondente
## Critérios de aceite (mapeados a itens da checklist E às regras do módulo 11,
   cada item verificável por um comando acima ou por observação objetiva —
   nunca por "parece certo" ou "está coerente")
...
## Plano de rollback (como reverter esta mudança específica caso quebre em
   qualquer blog da rede após rollout)
## Riscos e dependências de outros PRDs
## Estimativa de esforço (P/M/G)
```

Antes de considerar a Fase 1 concluída, releia cada PRD gerado e confirme:
faz sentido lido sozinho? Todo critério de aceite dá para verificar por
comando/observação objetiva? Se a resposta for "não" para qualquer PRD,
reescreva-o antes de seguir para a Fase 2.
...
- **Persistência incremental — obrigatório:** mantenha `docs/prd/STATUS.md`
  atualizado a cada artefato concluído (inventário, auditoria, cada PRD,
  roadmap). O arquivo deve conter, no mínimo: fase atual, artefatos já
  escritos, PRDs pendentes por número, data/hora da última atualização, e
  2-3 linhas sobre a próxima ação. Isso protege o trabalho contra queda de
  sessão ou compactação de contexto — qualquer sessão futura consegue
  retomar sem retrabalho apenas lendo o STATUS.md.
