        ...
        <div className="max-w-[1280px] mx-auto px-4 mt-6">
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <article className="w-full lg:w-2/3">
              ...
              {/* Header */}
              <span 
                className="inline-block text-white text-xs font-bold px-2.5 py-1 rounded-sm mb-4"
                style={{ backgroundColor: article.categoryColor }}
              >
                {article.category}
              </span>
              
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-[#1a2448] leading-tight mb-4 tracking-tight">
                {article.title}
              </h1>
            ...
            <aside className="w-full lg:w-1/3">
              <div className="sticky top-24">
                <div className="bg-gray-50 p-6 border border-gray-100 rounded-sm mb-8">
                  <h3 className="font-bold text-[#1a2448] text-lg mb-5 flex items-center gap-2 uppercase">
                      ...
                      <Link key={rel.id} href={`/artigo/${rel.id}`}>
                        <div className="flex gap-3 group cursor-pointer border-b border-gray-200 pb-4 last:border-0 last:pb-0">
                          ...
                          <div className="flex flex-col justify-center">
                            ...
                            <h4 className="text-sm font-bold text-[#1a2448] group-hover:text-[#1d4ed8] transition-colors leading-snug line-clamp-2">
                              {rel.title}
                            </h4>
                          </div>
                        </div>
                    ...
                    {moreArticles.slice(0, 3).map((item) => (
                      <Link key={item.id} href={`/artigo/${item.id}`}>
                        <div className="group cursor-pointer">
                          <span 
                            className="text-[10px] font-bold text-white px-1.5 py-0.5 rounded-sm inline-block mb-1"
                            style={{ backgroundColor: item.tagColor }}
                          >
                            {item.tag}
                          </span>
                          <h4 className="text-[15px] font-bold text-[#1a2448] group-hover:text-[#1d4ed8] transition-colors leading-snug">
                            {item.title}
                          </h4>
                        </div>
                      </Link>
