                    ...
                    {relatedArticles.map((rel) => (
                      <Link key={rel.id} href={`/artigo/${rel.id}`}>
                        <div className="flex gap-3 group cursor-pointer border-b border-gray-200 pb-4 last:border-0 last:pb-0">
                          <img src={rel.imageUrl} alt={rel.title} className="w-20 h-16 object-cover rounded-sm group-hover:opacity-80 shrink-0" />
                          <div className="flex flex-col justify-center">
                            <span className="text-[10px] font-bold text-gray-500 mb-1">{rel.time}</span>
                            <h4 className="text-sm font-bold text-[#1a2448] group-hover:text-[#1d4ed8] leading-snug line-clamp-2">{rel.title}</h4>
                          </div>
                        </div>
                      </Link>
                    ))}
                    ...
                    {moreArticles.slice(0, 3).map((item) => (
                      <Link key={item.id} href={`/artigo/${item.id}`}>
                        <div className="group cursor-pointer">
                          ...
                          <h4 className="text-[15px] font-bold text-[#1a2448] group-hover:text-[#1d4ed8] leading-snug">{item.title}</h4>
                        </div>
                      </Link>
                    ))}
                  </div>
