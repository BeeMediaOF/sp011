 ...
 *    `components/admin/editorBlocks.ts`; o que o schema não conhece o
 *    ProseMirror descarta ao reabrir o conteúdo.
 * 2. Artigo publicado pela central passa pelo sanitizador canônico do ingest
 *    (`@workspace/news-engine/sanitize`), que APAGA todo `style` inline. Por
 *    isso os blocos também levam `class` e o layout tem fallback em CSS
 *    (`.video-embed` no index.css) — o style inline é só conveniência.
 *
 * Espelhado em `central-web/src/lib/articleEmbeds.ts`.
 */
