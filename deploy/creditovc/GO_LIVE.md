...
Portal de **educação financeira e crédito** (pt-BR): sair das dívidas,
crédito, score, organizar finanças, renda extra, planejar o futuro e
investimentos. Nicho novo na rede — fontes próprias (proposta Bee Media),
**sem backfill** (o histórico da central é todo de esporte; o catálogo
nasce da coleta).
...
| Item | Valor |
|---|---|
| BLOG_ID / banco / bucket | `creditovc` |
| Domínio | `credito.vc` (zona própria — precisa de registro A) ou `creditovc.midia.run` (provisório, wildcard já resolve). **Defina na variável `DOMAIN` no topo de cada bloco.** |
| Idioma / fuso | pt-BR / America/Sao_Paulo (padrão — não mexer) |
| Categorias (slugs) | `sair-das-dividas`, `credito`, `score`, `organizar-financas`, `renda-extra`, `planejar-o-futuro`, `investimentos`, `outros` |
| Identidade | verde vivo `#0ec76d` / verde escuro `#0a9455` + navy `#0f2446`, rodapé `#0a1630`, wordmark "crédito**.vc**" (minúsculas), tagline "Educação financeira para a vida real" |
| Matcher SQL do blog | `name ILIKE '%credito%' OR name ILIKE '%crédito%' OR domain ILIKE '%credito%'` |

## 0) Pré-requisito: DNS

```bash
DOMAIN='creditovc.midia.run'
dig +short $DOMAIN
```

Deve devolver o IP da VPS. Subdomínio `.midia.run` já resolve pelo wildcard.
Zona própria (`credito.vc`): crie o registro **A do apex → IP da VPS** no
provedor do domínio ANTES de tudo (o Caddy emite o certificado sozinho
quando o DNS propagar).

## 1) Cadastro no painel central

Blogs → Novo:
- Nome: **Crédito.vc** · Domínio: o escolhido acima ·
...
docker compose exec -T pg-blogs psql -U postgres -c "CREATE ROLE creditovc_user LOGIN PASSWORD '$PASS';"
docker compose exec -T pg-blogs psql -U postgres -c "CREATE DATABASE creditovc OWNER creditovc_user;"
docker compose exec -T pg-blogs psql -U postgres -c "REVOKE CONNECT ON DATABASE creditovc FROM PUBLIC;"
```
...
mkdir -p /opt/blogs/creditovc && cd /opt/blogs/creditovc
cp /opt/sp011/deploy/blog-template/compose.yml .
cp /opt/sp011/deploy/blog-template/.env.example .env
sed -i "s|^BLOG_ID=.*|BLOG_ID=creditovc|" .env
sed -i "s|^COMPOSE_PROJECT_NAME=.*|COMPOSE_PROJECT_NAME=blog-creditovc|" .env
...
```bash
DOMAIN='creditovc.midia.run'
cd /opt/blogs/creditovc
docker compose up -d
printf '%s {\n\timport blog creditovc\n}\n' "$DOMAIN" > /opt/sp011/caddy/sites/creditovc.caddy
...
```bash
echo "postgresql://creditovc_user:$PASS@pg-blogs:5432/creditovc"
```

Em `https://<dominio>/admin/setup`:
- Token do passo 4.
- Connection string acima (se fechou o terminal, monte com a senha anotada:
  `postgresql://creditovc_user:<senha do passo 2>@pg-blogs:5432/creditovc`) ·
  SSL: **disable** (rede interna).
- Storage: URL + `service_role` do projeto de Storage + bucket `creditovc`.
...
O script cadastra as 10 fontes de finanças/economia da proposta (category
`financas` — categoria de fonte NOVA, não cruza com os blogs de esporte),
grava idioma/taxonomia do blog, cria a regra "Finanças (IA classifica)" e
**blinda o catch-all do sp011** (exclui `financas` — sem isso o sp011
receberia as 10 fontes de economia). Obs.: se o PontoFarma também estiver
no ar, a regra por palavra-chave dele passa a pescar pautas tributárias
destes mesmos feeds — é o esperado.
...
docker compose -f /opt/blogs/creditovc/compose.yml logs --tail 50
```

- 1º curl: `200` com TLS (ACME leva 10–60s; zona própria depende do DNS).
- 2º curl: `{"setupRequired":false}`.
- 3º curl: `<html lang="pt-BR"`.
- 4º curl: `"siteName":"Crédito.vc"` (diagnóstico anti-mistura de blogs).
