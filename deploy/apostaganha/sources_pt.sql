-- =============================================================================
-- Aposta Ganha — fontes PT (compartilhadas c/ Esporte Agora/Resenha Vip) +
-- taxonomia + regras
-- =============================================================================
...
--   2. Blog Aposta Ganha: language='pt-BR' + taxonomia (categories) — só grava
--      as categorias se ainda estiverem vazias.
--   3. Cria 8 regras de distribuição do Aposta Ganha (7 por categoria com
...
-- Aborta se o blog Aposta Ganha não estiver cadastrado no painel central
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM blogs WHERE name ILIKE '%aposta%ganha%' OR domain ILIKE '%apostaganha%'
  ) THEN
    RAISE EXCEPTION 'Blog Aposta Ganha nao encontrado em blogs — cadastre-o no painel central (Blogs -> Novo) antes de rodar este script.';
  END IF;
END $$;
...
-- -----------------------------------------------------------------------------
-- 2) Blog Aposta Ganha: idioma pt-BR (nunca traduz) + taxonomia p/ a IA
--    classificar as fontes gerais dentro do menu do blog
-- -----------------------------------------------------------------------------
UPDATE blogs SET language = 'pt-BR', updated_at = now()
WHERE (name ILIKE '%aposta%ganha%' OR domain ILIKE '%apostaganha%') AND language <> 'pt-BR';
...
WHERE (name ILIKE '%aposta%ganha%' OR domain ILIKE '%apostaganha%')
...
FROM blogs b
CROSS JOIN (VALUES
  ('Aposta Ganha - Copa do Mundo',         10, '["copa-do-mundo"]',     'copa-do-mundo'),
  ('Aposta Ganha - Futebol',               10, '["futebol"]',           'futebol'),
  ('Aposta Ganha - Vôlei',                 10, '["volei"]',             'volei'),
  ('Aposta Ganha - Tênis',                 10, '["tenis"]',             'tenis'),
  ('Aposta Ganha - Fórmula 1',             10, '["f1"]',                'f1'),
  ('Aposta Ganha - Futebol Americano',     10, '["futebol-americano"]', 'futebol-americano'),
  ('Aposta Ganha - e-Sports',              10, '["e-sports"]',          'e-sports'),
  ('Aposta Ganha - Outros (IA classifica)', 5, '["outros"]',            NULL)
) AS v(rule_name, priority, cats, target)
WHERE (b.name ILIKE '%aposta%ganha%' OR b.domain ILIKE '%apostaganha%')
  AND NOT EXISTS (
