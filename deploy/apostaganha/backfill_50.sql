-- =============================================================================
-- Aposta Ganha — backfill IMEDIATO: publica até 50 notícias de esporte do
...
-- ao Aposta Ganha é pulado. Idempotente: rodar 2x pega as PRÓXIMAS 50.
--
...
BEGIN
  ...
  WHERE name ILIKE '%aposta%ganha%' OR domain ILIKE '%apostaganha%' LIMIT 1;
  IF b.id IS NULL THEN
    RAISE EXCEPTION 'Blog Aposta Ganha nao encontrado — cadastre no painel central antes.';
  END IF;
  IF NOT b.is_active THEN
    RAISE EXCEPTION 'Blog Aposta Ganha esta INATIVO no painel — ative antes do backfill.';
  END IF;
  IF b.status <> 'online' THEN
    RAISE EXCEPTION 'Blog Aposta Ganha esta "%" (nao online) — rode "Testar conexao" no painel antes, senao as entregas morrem em retry.', b.status;
  END IF;
END $$;
...
candidates AS (
  ...
  WHERE ni.status = 'distributed'
    ...
    AND NOT EXISTS (                                   -- "não repetidas" no Aposta Ganha
...
\echo ''
\echo '=== BACKFILL AGENDADO (Aposta Ganha) ==='
