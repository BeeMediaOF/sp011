-- =============================================================================
-- Receba Bet — fontes PT (compartilhadas c/ Esporte Agora/Resenha Vip) +
-- taxonomia + regras
-- =============================================================================
...
--   2. Blog Receba Bet: language='pt-BR' + taxonomia (categories) — só grava
--      as categorias se ainda estiverem vazias.
--   3. Cria 8 regras de distribuição do Receba Bet (7 por categoria com
...
-- Aborta se o blog Receba Bet não estiver cadastrado no painel central
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM blogs WHERE name ILIKE '%receba%bet%' OR domain ILIKE '%recebabet%'
  ) THEN
    RAISE EXCEPTION 'Blog Receba Bet nao encontrado em blogs — cadastre-o no painel central (Blogs -> Novo) antes de rodar este script.';
  END IF;
END $$;
...
-- -----------------------------------------------------------------------------
-- 2) Blog Receba Bet: idioma pt-BR (nunca traduz) + taxonomia p/ a IA
--    classificar as fontes gerais dentro do menu do blog
-- -----------------------------------------------------------------------------
UPDATE blogs SET language = 'pt-BR', updated_at = now()
WHERE (name ILIKE '%receba%bet%' OR domain ILIKE '%recebabet%') AND language <> 'pt-BR';
...
WHERE (name ILIKE '%receba%bet%' OR domain ILIKE '%recebabet%')
...
FROM blogs b
CROSS JOIN (VALUES
  ('Receba Bet - Copa do Mundo',         10, '["copa-do-mundo"]',     'copa-do-mundo'),
  ('Receba Bet - Futebol',               10, '["futebol"]',           'futebol'),
  ('Receba Bet - Vôlei',                 10, '["volei"]',             'volei'),
  ('Receba Bet - Tênis',                 10, '["tenis"]',             'tenis'),
  ('Receba Bet - Fórmula 1',             10, '["f1"]',                'f1'),
  ('Receba Bet - Futebol Americano',     10, '["futebol-americano"]', 'futebol-americano'),
  ('Receba Bet - e-Sports',              10, '["e-sports"]',          'e-sports'),
  ('Receba Bet - Outros (IA classifica)', 5, '["outros"]',            NULL)
) AS v(rule_name, priority, cats, target)
WHERE (b.name ILIKE '%receba%bet%' OR b.domain ILIKE '%recebabet%')
  AND NOT EXISTS (
