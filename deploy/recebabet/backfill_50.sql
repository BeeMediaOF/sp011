-- =============================================================================
-- Receba Bet — backfill IMEDIATO: publica até 50 notícias de esporte do
...
-- ao Receba Bet é pulado. Idempotente: rodar 2x pega as PRÓXIMAS 50.
--
...
BEGIN
  ...
  WHERE name ILIKE '%receba%bet%' OR domain ILIKE '%recebabet%' LIMIT 1;
  IF b.id IS NULL THEN
    RAISE EXCEPTION 'Blog Receba Bet nao encontrado — cadastre no painel central antes.';
  END IF;
  IF NOT b.is_active THEN
    RAISE EXCEPTION 'Blog Receba Bet esta INATIVO no painel — ative antes do backfill.';
  END IF;
  IF b.status <> 'online' THEN
    RAISE EXCEPTION 'Blog Receba Bet esta "%" (nao online) — rode "Testar conexao" no painel antes, senao as entregas morrem em retry.', b.status;
  END IF;
END $$;
...
candidates AS (
  ...
  WHERE ni.status = 'distributed'
    ...
    AND NOT EXISTS (                                   -- "não repetidas" no Receba Bet
...
\echo ''
\echo '=== BACKFILL AGENDADO (Receba Bet) ==='
