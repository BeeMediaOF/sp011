# Receba Bet — go-live (recebabet.midia.run)

Mais um blog de **esporte em português**, irmão do Esporte Agora, Resenha Vip,
Oley Sports, Bee Esportes e Aposta Ganha — mesmas fontes, mesmos slugs, template
...
| Item | Valor |
|---|---|
| BLOG_ID / banco / bucket | `recebabet` |
| Domínio | `recebabet.midia.run` (ajuste se for zona própria — os scripts SQL localizam o blog por nome `%receba%bet%` ou domínio `%recebabet%`) |
...
| Identidade (logo) | azul-céu `#3d9bff` (topo do gradiente da logo) · azul royal `#0f62d6` · navy `#071b3d` · rodapé `#040f26` |
...
Blogs → Novo:
- Nome: **Receba Bet** · Domínio: `recebabet.midia.run` ·
  API URL: `https://recebabet.midia.run`
...
docker compose exec -T pg-blogs psql -U postgres -c "CREATE ROLE recebabet_user LOGIN PASSWORD '$PASS';"
docker compose exec -T pg-blogs psql -U postgres -c "CREATE DATABASE recebabet OWNER recebabet_user;"
docker compose exec -T pg-blogs psql -U postgres -c "REVOKE CONNECT ON DATABASE recebabet FROM PUBLIC;"
```
...
mkdir -p /opt/blogs/recebabet && cd /opt/blogs/recebabet
cp /opt/sp011/deploy/blog-template/compose.yml .
cp /opt/sp011/deploy/blog-template/.env.example .env
sed -i "s|^BLOG_ID=.*|BLOG_ID=recebabet|" .env
sed -i "s|^COMPOSE_PROJECT_NAME=.*|COMPOSE_PROJECT_NAME=blog-recebabet|" .env
sed -i "s|^BLOG_IMAGE_TAG=.*|BLOG_IMAGE_TAG=$TAG|" .env
sed -i "s|^APP_URL=.*|APP_URL=https://recebabet.midia.run|" .env
sed -i "s|^SITE_URL=.*|SITE_URL=https://recebabet.midia.run|" .env
sed -i "s|^ALLOWED_ORIGINS=.*|ALLOWED_ORIGINS=https://recebabet.midia.run|" .env
...
```bash
cd /opt/blogs/recebabet
docker compose up -d
printf 'recebabet.midia.run {\n\timport blog recebabet\n}\n' > /opt/sp011/caddy/sites/recebabet.caddy
...
```bash
echo "postgresql://recebabet_user:$PASS@pg-blogs:5432/recebabet"
```

Em `https://recebabet.midia.run/admin/setup`:
- Token do passo 4.
- Connection string acima (se fechou o terminal, monte com a senha anotada:
  `postgresql://recebabet_user:<senha do passo 2>@pg-blogs:5432/recebabet`) ·
  SSL: **disable** (rede interna).
- Storage: URL + `service_role` do projeto de Storage + bucket `recebabet`.
...
docker compose exec -T pg-blogs psql "$DBURL" -v ON_ERROR_STOP=1 < deploy/recebabet/sources_pt.sql
```

As 16 fontes são as MESMAS dos irmãos de esporte (upsert é no-op se já
existem); o script grava idioma/taxonomia do blog e cria as 8 regras do Receba
...
Em `https://recebabet.midia.run/admin` (o template aparece em Home + menu →
aba Templates → "Meus templates" em ≤15s, sem restart):

1. Aplicar **"Receba Bet - Final"** — instala os 22 blocos, menu PT, rodapé
   navy, banners "Anuncie aqui" azul, idioma pt-BR + fuso SP.
2. **Configurações → Informações**: nome "Receba Bet", tagline, **upload das
   logos** (horizontal p/ header, ícone/quadrado p/ favicon), autor padrão
   (ex.: "Redação Receba Bet"). Nunca reaproveitar caminho `/api/uploads/`
...
docker compose exec -T pg-blogs psql -U postgres -d recebabet -v ON_ERROR_STOP=1 < deploy/recebabet/social_feed_story.sql
```
...
cd /opt/sp011
DBURL=$(grep -m1 '^CENTRAL_DATABASE_URL=' .env.central | cut -d= -f2- | sed -e 's/^"//' -e 's/"$//')
docker compose exec -T pg-blogs psql "$DBURL" -v ON_ERROR_STOP=1 < deploy/recebabet/backfill_50.sql
```
...
```bash
curl -sI https://recebabet.midia.run
curl -s https://recebabet.midia.run/api/setup
curl -s https://recebabet.midia.run/ | grep -o '<html lang="[^"]*"'
curl -s https://recebabet.midia.run/api/site | grep -o '"siteName":"[^"]*"'
...
- 4º curl: **`Receba Bet`** — se sair o nome de outro blog, é mistura de
