-- =============================================================================
-- Cassino Bet — backfill IMEDIATO: publica até 50 notícias de esporte do
...
-- ao Cassino Bet é pulado. Idempotente: rodar 2x pega as PRÓXIMAS 50.
--
...
BEGIN
  ...
  WHERE name ILIKE '%cassino%bet%' OR domain ILIKE '%cassinobet%' LIMIT 1;
  IF b.id IS NULL THEN
    RAISE EXCEPTION 'Blog Cassino Bet nao encontrado — cadastre no painel central antes.';
  END IF;
  IF NOT b.is_active THEN
    RAISE EXCEPTION 'Blog Cassino Bet esta INATIVO no painel — ative antes do backfill.';
  END IF;
  IF b.status <> 'online' THEN
    RAISE EXCEPTION 'Blog Cassino Bet esta "%" (nao online) — rode "Testar conexao" no painel antes, senao as entregas morrem em retry.', b.status;
  END IF;
END $$;
...
candidates AS (
  ...
  WHERE ni.status = 'distributed'
    ...
    AND NOT EXISTS (                                   -- "não repetidas" no Cassino Bet
...
\echo ''
\echo '=== BACKFILL AGENDADO (Cassino Bet) ==='
