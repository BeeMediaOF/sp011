-- =============================================================================
-- Bee Esportes — fontes PT (compartilhadas c/ Esporte Agora/Resenha Vip) +
-- taxonomia + regras
-- =============================================================================
-- O que faz (idempotente — rodar 2x é seguro):
--   1. Garante as mesmas 16 fontes PT de esporte em `central_sources`
--      (upsert por URL — se o sources_pt.sql do EA/RV/Oley já rodou, esta
--      etapa é no-op; as fontes são COMPARTILHADAS entre os blogs).
--   2. Blog Bee Esportes: language='pt-BR' + taxonomia (categories) — só grava
--      as categorias se ainda estiverem vazias.
--   3. Cria 8 regras de distribuição do Bee Esportes (7 por categoria com
...
--
-- ATENÇÃO ao matcher: o ksports mora em ksports.bebee.me ("bee" no domínio!)
-- — por isso este script localiza o blog por name ILIKE '%bee%esporte%' OR
-- domain ILIKE '%beeesportes%', que NÃO casa com o ksports.
--
...
-- Aborta se o blog Bee Esportes não estiver cadastrado no painel central
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM blogs WHERE name ILIKE '%bee%esporte%' OR domain ILIKE '%beeesportes%'
  ) THEN
    RAISE EXCEPTION 'Blog Bee Esportes nao encontrado em blogs — cadastre-o no painel central (Blogs -> Novo) antes de rodar este script.';
  END IF;
END $$;
...
-- -----------------------------------------------------------------------------
-- 2) Blog Bee Esportes: idioma pt-BR (nunca traduz) + taxonomia p/ a IA
--    classificar as fontes gerais dentro do menu do blog
-- -----------------------------------------------------------------------------
UPDATE blogs SET language = 'pt-BR', updated_at = now()
WHERE (name ILIKE '%bee%esporte%' OR domain ILIKE '%beeesportes%') AND language <> 'pt-BR';
...
WHERE (name ILIKE '%bee%esporte%' OR domain ILIKE '%beeesportes%')
...
FROM blogs b
CROSS JOIN (VALUES
  ('Bee Esportes - Copa do Mundo',         10, '["copa-do-mundo"]',     'copa-do-mundo'),
  ('Bee Esportes - Futebol',               10, '["futebol"]',           'futebol'),
  ('Bee Esportes - Vôlei',                 10, '["volei"]',             'volei'),
  ('Bee Esportes - Tênis',                 10, '["tenis"]',             'tenis'),
  ('Bee Esportes - Fórmula 1',             10, '["f1"]',                'f1'),
  ('Bee Esportes - Futebol Americano',     10, '["futebol-americano"]', 'futebol-americano'),
  ('Bee Esportes - e-Sports',              10, '["e-sports"]',          'e-sports'),
  ('Bee Esportes - Outros (IA classifica)', 5, '["outros"]',            NULL)
) AS v(rule_name, priority, cats, target)
WHERE (b.name ILIKE '%bee%esporte%' OR b.domain ILIKE '%beeesportes%')
  AND NOT EXISTS (
