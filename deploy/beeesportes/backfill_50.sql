-- =============================================================================
-- Bee Esportes — backfill IMEDIATO: publica até 50 notícias de esporte do
-- histórico, reaproveitando as reescritas do Esporte Agora (roda no CENTRAL)
-- =============================================================================
-- Mesmo padrão do backfill_50 do Resenha Vip — NADA passa pela IA:
...
--
-- Matcher: name ILIKE '%bee%esporte%' OR domain ILIKE '%beeesportes%' —
-- NÃO casa com o ksports (ksports.bebee.me), de propósito.
--
-- Dedupe: NOT EXISTS + UNIQUE (news_item_id, blog_id) — o que já foi entregue
-- ao Bee Esportes é pulado. Idempotente: rodar 2x pega as PRÓXIMAS 50.
--
...
BEGIN
  ...
  WHERE name ILIKE '%bee%esporte%' OR domain ILIKE '%beeesportes%' LIMIT 1;
  IF b.id IS NULL THEN
    RAISE EXCEPTION 'Blog Bee Esportes nao encontrado — cadastre no painel central antes.';
  END IF;
  IF NOT b.is_active THEN
    RAISE EXCEPTION 'Blog Bee Esportes esta INATIVO no painel — ative antes do backfill.';
  END IF;
  IF b.status <> 'online' THEN
    RAISE EXCEPTION 'Blog Bee Esportes esta "%" (nao online) — rode "Testar conexao" no painel antes, senao as entregas morrem em retry.', b.status;
  END IF;
END $$;
...
candidates AS (
  ...
  WHERE ni.status = 'distributed'
    ...
    AND NOT EXISTS (                                   -- "não repetidas" no Bee
...
\echo ''
\echo '=== BACKFILL AGENDADO (Bee Esportes) ==='
