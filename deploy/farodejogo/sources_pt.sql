-- =============================================================================
-- Faro de Jogo — fontes PT (compartilhadas c/ Esporte Agora/Resenha Vip) +
-- taxonomia + regras
-- =============================================================================
...
--   2. Blog Faro de Jogo: language='pt-BR' + taxonomia (categories) — só grava
--      as categorias se ainda estiverem vazias.
--   3. Cria 8 regras de distribuição do Faro de Jogo (7 por categoria com
...
-- Aborta se o blog Faro de Jogo não estiver cadastrado no painel central
DO $$
BEGIN
  IF NOT EXISTS (
...
-- -----------------------------------------------------------------------------
-- 2) Blog Faro de Jogo: idioma pt-BR (nunca traduz) + taxonomia p/ a IA
--    classificar as fontes gerais dentro do menu do blog
-- -----------------------------------------------------------------------------
UPDATE blogs SET language = 'pt-BR', updated_at = now()
WHERE (name ILIKE '%faro%de%jogo%' OR domain ILIKE '%farodejogo%') AND language <> 'pt-BR';
...
FROM blogs b
CROSS JOIN (VALUES
  ('Faro de Jogo - Copa do Mundo',         10, '["copa-do-mundo"]',     'copa-do-mundo'),
  ('Faro de Jogo - Futebol',               10, '["futebol"]',           'futebol'),
  ('Faro de Jogo - Vôlei',                 10, '["volei"]',             'volei'),
  ('Faro de Jogo - Tênis',                 10, '["tenis"]',             'tenis'),
  ('Faro de Jogo - Fórmula 1',             10, '["f1"]',                'f1'),
  ('Faro de Jogo - Futebol Americano',     10, '["futebol-americano"]', 'futebol-americano'),
  ('Faro de Jogo - e-Sports',              10, '["e-sports"]',          'e-sports'),
  ('Faro de Jogo - Outros (IA classifica)', 5, '["outros"]',            NULL)
