...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM settings WHERE key = 'site_settings') THEN
    RAISE EXCEPTION 'site_settings nao existe neste banco — conclua o wizard /admin/setup do Faro de Jogo antes de importar o template.';
  END IF;
END $$;
...
SET value = jsonb_set(
      ...
      (
        ...
        WHERE t->>'id' <> 'tpl-farodejogo-final'
      ) || $tpl$
