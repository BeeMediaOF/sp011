-- =============================================================================
-- Resenha Vip — fontes PT (compartilhadas c/ Esporte Agora) + taxonomia + regras
-- =============================================================================
-- O que faz (idempotente — rodar 2x é seguro):
--   1. Garante as mesmas 16 fontes PT de esporte do Esporte Agora em
--      `central_sources` (upsert por URL — se o sources_pt.sql do EA já rodou,
--      esta etapa é no-op; as fontes são COMPARTILHADAS entre os blogs).
--   2. Blog Resenha Vip: language='pt-BR' + taxonomia (categories) — só grava
--      as categorias se ainda estiverem vazias.
--   3. Cria 8 regras de distribuição do Resenha Vip (7 por categoria com
...
--
-- Slugs IGUAIS aos do Esporte Agora (ambos pt-BR — não há risco de tradução;
-- a mesma notícia passa a ser entregue aos DOIS blogs). Notícia de fonte
-- genérica ("outros") é classificada uma vez POR BLOG (chamada barata,
-- título+resumo). O catch-all do sp011 já exclui esses 8 slugs desde
-- 2026-07-10 — nada a fazer lá.
--
-- Uso (VPS, /opt/sp011):
...
-- Aborta se o blog Resenha Vip não estiver cadastrado no painel central
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM blogs WHERE name ILIKE '%resenha%' OR domain ILIKE '%resenhavip%'
  ) THEN
    RAISE EXCEPTION 'Blog Resenha Vip nao encontrado em blogs — cadastre-o no painel central (Blogs -> Novo) antes de rodar este script.';
  END IF;
END $$;

-- -----------------------------------------------------------------------------
-- 1) Fontes PT (as mesmas do Esporte Agora; upsert por URL; prompt padrão)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2) Blog Resenha Vip: idioma pt-BR (nunca traduz) + taxonomia p/ a IA
--    classificar as fontes gerais dentro do menu do blog
-- -----------------------------------------------------------------------------
UPDATE blogs SET language = 'pt-BR', updated_at = now()
WHERE (name ILIKE '%resenha%' OR domain ILIKE '%resenhavip%') AND language <> 'pt-BR';
...
FROM blogs b
CROSS JOIN (VALUES
  ('Resenha Vip - Copa do Mundo',         10, '["copa-do-mundo"]',     'copa-do-mundo'),
  ('Resenha Vip - Futebol',               10, '["futebol"]',           'futebol'),
  ('Resenha Vip - Vôlei',                 10, '["volei"]',             'volei'),
  ('Resenha Vip - Tênis',                 10, '["tenis"]',             'tenis'),
  ('Resenha Vip - Fórmula 1',             10, '["f1"]',                'f1'),
  ('Resenha Vip - Futebol Americano',     10, '["futebol-americano"]', 'futebol-americano'),
  ('Resenha Vip - e-Sports',              10, '["e-sports"]',          'e-sports'),
  ('Resenha Vip - Outros (IA classifica)', 5, '["outros"]',            NULL)
