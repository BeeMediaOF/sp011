-- =============================================================================
-- Oley Sports — fontes PT (compartilhadas c/ Esporte Agora/Resenha Vip) +
-- taxonomia + regras
-- =============================================================================
...
--   2. Blog Oley Sports: language='pt-BR' + taxonomia (categories) — só grava
--      as categorias se ainda estiverem vazias.
--   3. Cria 8 regras de distribuição do Oley Sports (7 por categoria com
...
-- Aborta se o blog Oley Sports não estiver cadastrado no painel central
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM blogs WHERE name ILIKE '%oley%' OR domain ILIKE '%oleysports%'
  ) THEN
    RAISE EXCEPTION 'Blog Oley Sports nao encontrado em blogs — cadastre-o no painel central (Blogs -> Novo) antes de rodar este script.';
  END IF;
END $$;
...
-- -----------------------------------------------------------------------------
-- 2) Blog Oley Sports: idioma pt-BR (nunca traduz) + taxonomia p/ a IA
--    classificar as fontes gerais dentro do menu do blog
-- -----------------------------------------------------------------------------
UPDATE blogs SET language = 'pt-BR', updated_at = now()
WHERE (name ILIKE '%oley%' OR domain ILIKE '%oleysports%') AND language <> 'pt-BR';
...
WHERE (name ILIKE '%oley%' OR domain ILIKE '%oleysports%')
...
FROM blogs b
CROSS JOIN (VALUES
  ('Oley Sports - Copa do Mundo',         10, '["copa-do-mundo"]',     'copa-do-mundo'),
  ('Oley Sports - Futebol',               10, '["futebol"]',           'futebol'),
  ('Oley Sports - Vôlei',                 10, '["volei"]',             'volei'),
  ('Oley Sports - Tênis',                 10, '["tenis"]',             'tenis'),
  ('Oley Sports - Fórmula 1',             10, '["f1"]',                'f1'),
  ('Oley Sports - Futebol Americano',     10, '["futebol-americano"]', 'futebol-americano'),
  ('Oley Sports - e-Sports',              10, '["e-sports"]',          'e-sports'),
  ('Oley Sports - Outros (IA classifica)', 5, '["outros"]',            NULL)
) AS v(rule_name, priority, cats, target)
WHERE (b.name ILIKE '%oley%' OR b.domain ILIKE '%oleysports%')
  AND NOT EXISTS (
