/**
 * safeFetch — fetch com defesa contra SSRF (PRD-06b) — ESPELHO do util do
 * api-server (`artifacts/api-server/src/lib/safeFetch.ts`, PRD-06a).
 *
 * Por que uma cópia: `lib/news-engine` é um pacote composite de camada mais
 * baixa (o api-server depende dele, nunca o contrário), então `scrape.ts` não
 * pode importar `../../artifacts/api-server/...`. Este arquivo mantém a MESMA
 * semântica do original — qualquer mudança deve ser espelhada nos dois.
 *
 * Garantias:
 ...
 *
 * Resíduo aceito (documentado): janela TOCTOU de DNS-rebinding entre a
 * resolução do DNS e o connect real. O bloqueio de redirect + a resolução de
 * DNS fecham o vetor prático do AP-2.
 */
